...
META_RANK = 'rank'
META_DEFAULT_TITLE = 'Untitled'
META_DEFAULT_CATEGORY = 'UNCLASSIFIED'
META_DOCUMENT_DEFAULT_CATEGORY = 'OTHER DOCUMENTS'
META_DEFAULT_RANK = 0
...
# Document Settings
def from_file(path):
    with io.open(path, 'r', encoding=ENCODING) as reader:
        return reader.read()

CONTENT_TEMPLATE = '<div class="toc">{toc}</div>{separator}{document}'

TOC_CATEGORY_TEMPLATE = '<h3 id="{category_md5}"><div class="left">{category}</div><div class="right">□</div></h3>'

TOC_TITLE_TEMPLATE = '<h4 class="title"><b><a href="#{id}">{id}</a></b>. {title}</h4>'

DOCUMENT_TEMPLATE = '''<div class="source-code">
...
UNUSED_DOCUMENT_TEMPLATE = '''<div class="document">
...
# Using KaTeX v0.11.0
WEBPAGE_TEMPLATE = '''<!DOCTYPE html><html><head>
  <meta charset="UTF-8">
